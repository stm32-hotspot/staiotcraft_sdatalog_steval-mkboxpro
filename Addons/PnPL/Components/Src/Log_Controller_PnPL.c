...
char *Log_Controller_PnPL_vtblGetCommandKey(IPnPLComponent_t *_this, uint8_t id)
{
  ...
  {
    case 0:
      return "log_controller*save_configuration";
      break;
    case 1:
                                          ...
                                          uint8_t pretty)
{
  ...
  json_object_dotset_number(JSON_Status, "log_controller.c_type", COMP_TYPE_APPLICATION);
...
uint8_t Log_Controller_PnPL_vtblExecuteFunction(IPnPLComponent_t *_this, char *serializedJSON)
{
  ...
  if (json_object_dothas_value(tempJSONObject, "log_controller*save_configuration"))
  {
    ret = log_controller_save_configuration();
  }
