...
#define ASPEP_PAYLOAD_TYPE_HEADER_MASK  0x000007FF
...
#define ASPEP_TX_COMPLETE_CALLBACK(A) \
  if((A)->ASPEP_tx_complete_cb != NULL) \
    (A)->ASPEP_tx_complete_cb();

#define ASPEP_PACKET_RECEIVED_CALLBACK(A) \
  if((A)->ASPEP_PacketReceived_cb != NULL) \
    (A)->ASPEP_PacketReceived_cb();
...
void ASPEP_sendBeacon(ASPEP_Handle_t *pHandle, ASPEP_Capabilities_def *capabilities)
{
  ...
  else
  {
    ...
    *packet = (BEACON | (((uint32_t) capabilities->version) << 4U) | (((uint32_t) capabilities->DATA_CRC) << 7U) | (((uint32_t) capabilities->RX_maxSize) << 8U)
               | (((uint32_t) capabilities->TXS_maxSize) << 14U) | (((uint32_t) capabilities->TXA_maxSize) << 21U));
...
void ASPEP_sendPing(ASPEP_Handle_t *pHandle, uint8_t cBit, uint16_t packetNumber)
{
  ...
  else
  {
    ...
    *packet = PING | (uint32_t)((uint32_t) cBit << 4U) | (uint32_t)((uint32_t) cBit << 5U) | (uint32_t)((uint32_t) Nbit << 6U)
              | (uint32_t)((uint32_t) Nbit << 7U) | (uint32_t)((uint32_t) ipID << 8U) | (uint32_t)(((uint32_t) packetNumber) << 12U);
    (void) ASPEP_TXframeProcess(pHandle, ASPEP_CTRL, &pHandle->ctrlBuffer, ASPEP_CTRL_SIZE);
#ifdef NULL_PTR_ASP
  }
#endif
...
bool ASPEP_getBuffer(ASPEPTL_Handle_t *pSupHandle, void **buffer, uint8_t syncAsync)
{
  bool result = true;
#ifdef NULL_PTR_ASP
  if (MC_NULL == buffer)
...
uint8_t ASPEP_sendPacket(ASPEPTL_Handle_t *pSupHandle, void *txBuffer, uint16_t txDataLength, uint8_t syncAsync)
{
  ...
  else
  {
    ...
    if (ASPEP_CONNECTED == pHandle->ASPEP_State)
    {
      ...
      header--; /* Header uses 4*8 bits on top of txBuffer*/
      *header = ((uint32_t)((uint32_t) txDataLengthTemp << (uint32_t) 4) | (uint32_t) syncAsync);

      *header = *header | ((ASPEP_PAYLOAD_TYPE & ASPEP_PAYLOAD_TYPE_HEADER_MASK) << 17U);
      ...
      if (ASPEP_SYNC == syncAsync)
      {
        if (pSupHandle->packetAvailable)
        {
          pSupHandle->packetAvailable = false; /* CMD from master is processed*/
          if (txDataLength > ASPEP_TX_SYNC_PAYLOAD_MAX)
          {
            result = ASPEP_BUFFER_ERROR;
          }
        }
        ...
        {
          result = ASPEP_SYNC_NOT_EXPECTED;
        }
      }
      else /* ASYNC */
      {
        if (txDataLength > ASPEP_TX_ASYNC_PAYLOAD_MAX_A)
        {
          result = ASPEP_BUFFER_ERROR;
        }
      }
...
/*
 ...
 It can be accessed concurrently under High frequency task (by MCPA_datalog)
...
void ASPEP_HWDataTransmittedIT(ASPEP_Handle_t *pHandle)
{
  ...
  else
  {
    ...
    else /* if previous buffer was not ASPEP_CTRL, then the buffer locked is a ASPEPTL_Buff_t */
    {
#if ASPEP_USE_EXTERNAL_ASYNC_BUFFERS == 1
      if (pHandle->packet_type == ASPEP_ASYNC)
      {
        ASPEP_TX_COMPLETE_CALLBACK(pHandle);
      }
      else
#endif
      {
        ASPEPTL_Buff_t *tempBuff = (ASPEPTL_Buff_t *) pHandle->lockBuffer; //cstat !MISRAC2012-Rule-11.5
    ...
    else
    {
      ...
      if (pHandle->asyncNextBuffer != NULL)
      {
        ...
        if ((pHandle->asyncBufferA.state == pending) || (pHandle->asyncBufferB.state == pending))
        {
          ...
          pHandle->asyncNextBuffer = (ASPEPTL_Buff_t *) temp; //cstat !MISRAC2012-Rule-11.4
        }
        else
        {
...
uint8_t *ASPEP_RXframeProcess(ASPEPTL_Handle_t *pSupHandle, uint16_t *packetLength)
{
  uint8_t *result = NULL;
#ifdef NULL_PTR_ASP
        ...
        case ASPEP_CONNECTED:
        {
          ...
          else if (DATA_PACKET == pHandle->rxPacketType)
          {
            if (validCRCData)
            {
              ...
              pSupHandle->packetAvailable = true; /* Will be consumed in ASPEP_sendPacket */
          ...
          {
            /* This condition is not reachable because already filtered by NewPacketAvailable */
          ...
          switch (pHandle->rxPacketType)
          {
            case DATA_PACKET:
            {
              ...
              if (0U == pHandle->rxLength) /* data packet with length 0 is a valid packet */
              {
                ...
                ASPEP_PACKET_RECEIVED_CALLBACK(pHandle);

                /* The receiver is not reconfigured right now on purpose to avoid race condition when the packet will be
...
void ASPEP_RegisterNewPacket_cb(ASPEP_Handle_t *pHandle, ASPEP_packet_received_cb_t ASPEP_PacketReceived_cb)
{
  if (NULL != pHandle && NULL != ASPEP_PacketReceived_cb)
  {
    pHandle->ASPEP_PacketReceived_cb = ASPEP_PacketReceived_cb;
  }
}

void ASPEP_RegisterTxComplete_cb(ASPEP_Handle_t *pHandle, ASPEP_tx_complete_cb_t ASPEP_tx_complete_cb)
{
  if (NULL != pHandle && NULL != ASPEP_tx_complete_cb)
  {
    pHandle->ASPEP_tx_complete_cb = ASPEP_tx_complete_cb;
  }
}
