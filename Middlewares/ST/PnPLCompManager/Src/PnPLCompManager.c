...
/* Get Device UUID string. If not replaced at deploy time, this returns an empty string. */
const char *PnPLGetDeviceUUID(void)
{
  if(PnPLHasValidDeviceUUID()) return (const char*)device_uuid.pnpl_device_uuid;
  return (const char*)"";
}
...
static void PnPLSeedRandOnce(void);
...
static void PnPLSeedRandOnce(void)
{
  ...
  uint32_t uid0 = (uint32_t)(READ_REG(*((uint32_t *)UID_BASE)));
  uint32_t uid1 = (uint32_t)(READ_REG(*((uint32_t *)(UID_BASE + 4U))));
  uint32_t uid2 = (uint32_t)(READ_REG(*((uint32_t *)(UID_BASE + 8U))));
  uint32_t seed = uid0 ^ (uid1 << 7) ^ (uid2 >> 3);

  /* Mix in runtime jitter so sequence is less predictable across resets. */
  seed ^= HAL_GetTick();
  seed ^= SysTick->VAL;
...
uint8_t PnPLGetDeviceStatusJSON(char **serializedJSON, uint32_t *size, uint8_t pretty)
{
  ...
  */
  ...
  (void)json_object_dotset_number(JSON_Device, "protocol_id", (float_t)PnPLGetPROTOCOLID()); /* 0: BLE, 1: serial, 2:libusb */
